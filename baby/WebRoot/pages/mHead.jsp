<%@ page language="java" import="java.util.*" pageEncoding="utf-8"%>

<link href="/resources/css/jquery-ui-themes.css" type="text/css" rel="stylesheet"/>
<link href="/resources/css/axure_rp_page.css" type="text/css" rel="stylesheet"/>
<link href="/data/styles.css" type="text/css" rel="stylesheet"/>
<link href="/files/manage/styles.css" type="text/css" rel="stylesheet"/>
<style>

.div_main{width:900px; margin: 0 auto;position: absolute;}
</style>
<script type="text/javascript">
$(function(){
	$(".div_main").css("left", (document.documentElement.clientWidth-900)/2);
	$(window).resize(function(){
		$(".div_main").css("left", (document.documentElement.clientWidth-900)/2);
	});
});
</script>
<script src="/pages/js/mHead.js"></script>
<style type="text/css">
	.ax_形状{cursor: pointer;}
</style>
<div>
	 <!-- 管理员 (动态面板) -->
      <div id="u0" class="ax_动态面板" style="width: 900px;" data-label="管理员">
        <div id="u0_state0" class="panel_state" data-label="实时">
          <div id="u0_state0_content" class="panel_state_content">

            <!-- Unnamed (形状) -->
            <div id="u1" class="ax_形状">
              <img id="u1_img" class="img " style="width: 900px;" src="/images/manage/u1.png"/>
              <!-- Unnamed () -->
              <div id="u2" class="text">
                <p><span></span></p>
              </div>
            </div>

            <!-- 实时-实时 (形状) -->
            <div id="u3" class="ax_形状 mhead" data-label="实时-实时"  onclick="turn('/userMonitorAction_actualTimeMonitor.action');">
              <img id="u3_img" class="img " src="/images/manage/u3.png"/>
              <!-- Unnamed () -->
              <div id="u4" class="text">
                <p><span>实时</span></p>
              </div>
            </div>

            <!-- 实时-实时-水平线 (水平线) -->
            <div id="u5" class="ax_水平线" data-label="实时-实时-水平线" style="display: none;">
              <img id="u5_start" class="img " src="/resources/images/transparent.gif" alt="u5_start"/>
              <img id="u5_end" class="img " src="/resources/images/transparent.gif" alt="u5_end"/>
              <img id="u5_line" class="img " src="/images/manage/u5_line.png" alt="u5_line"/>
            </div>

            <!-- 实时-用户管理 (形状) -->
            <div id="u6" class="ax_形状 mhead" data-label="实时-用户管理"  onclick="turn('/userManageAction_initUserManage.action');">
              <img id="u6_img" class="img " src="/resources/images/transparent.gif"/>
              <!-- Unnamed () -->
              <div id="u7" class="text">
                <p><span>用户管理</span></p>
              </div>
            </div>

            <!-- 实时-用户管理-水平线 (水平线) -->
            <div id="u8" class="ax_水平线" data-label="实时-用户管理-水平线" style="display: none;">
              <img id="u8_start" class="img " src="/resources/images/transparent.gif" alt="u8_start"/>
              <img id="u8_end" class="img " src="/resources/images/transparent.gif" alt="u8_end"/>
              <img id="u8_line" class="img " src="/images/manage/u8_line.png" alt="u8_line"/>
            </div>

            <!-- 实时-设备管理 (形状) -->
            <div id="u9" class="ax_形状 mhead" data-label="实时-设备管理"  onclick="turn('/monitorManageAction_initMonitorManage.action');">
              <img id="u9_img" class="img " src="/resources/images/transparent.gif"/>
              <!-- Unnamed () -->
              <div id="u10" class="text">
                <p><span>设备管理</span></p>
              </div>
            </div>

            <!-- 实时-设备管理-水平线 (水平线) -->
            <div id="u11" class="ax_水平线" data-label="实时-设备管理-水平线" style="display: none;">
              <img id="u11_start" class="img " src="/resources/images/transparent.gif" alt="u11_start"/>
              <img id="u11_end" class="img " src="/resources/images/transparent.gif" alt="u11_end"/>
              <img id="u11_line" class="img " src="/images/manage/u8_line.png" alt="u11_line"/>
            </div>

            <!-- 实时-个人中心 (形状) -->
            <div id="u12" class="ax_形状 mhead" data-label="实时-个人中心" onclick="turn('personalCenterAction_initPersonalCenter.action');">
              <img id="u12_img" class="img " src="/resources/images/transparent.gif"/>
              <!-- Unnamed () -->
              <div id="u13" class="text">
                <p><span>个人中心</span></p>
              </div>
            </div>

            <!-- 实时-个人中心-水平线 (水平线) -->
            <div id="u14" class="ax_水平线" data-label="实时-个人中心-水平线" style="display: none;">
              <img id="u14_start" class="img " src="/resources/images/transparent.gif" alt="u14_start"/>
              <img id="u14_end" class="img " src="/resources/images/transparent.gif" alt="u14_end"/>
              <img id="u14_line" class="img " src="/images/manage/u8_line.png" alt="u14_line"/>
            </div>

            <!-- 推送 (形状) -->
            <div id="u15" class="ax_形状 mhead" data-label="推送">
              <img id="u15_img" class="img " src="/resources/images/transparent.gif"/>
              <!-- Unnamed () -->
              <div id="u16" class="text">
                <p><span>推送</span></p>
              </div>
            </div>

            <!-- 实时-推送-水平线 (水平线) -->
            <div id="u17" class="ax_水平线" data-label="实时-推送-水平线" style="display: none;">
              <img id="u17_start" class="img " src="/resources/images/transparent.gif" alt="u17_start"/>
              <img id="u17_end" class="img " src="/resources/images/transparent.gif" alt="u17_end"/>
              <img id="u17_line" class="img " src="/images/manage/u5_line.png" alt="u17_line"/>
            </div>
            
             <!-- 实时-退出登录 (形状) -->
            <div id="u151" class="ax_形状 mhead" data-label="实时-退出登录" onclick="turn('loginAction_loginoff.action');">
              <img id="u151_img" class="img " src="/resources/images/transparent.gif"/>
              <!-- Unnamed () -->
              <div id="u152" class="text">
                <p><span>退出登录</span></p>
              </div>
            </div>

            <!-- 实时-退出登录-水平线 (水平线) -->
            <div id="u153" class="ax_水平线" data-label="实时-退出登录-水平线" style="display: none;">
              <img id="u153_start" class="img " src="/resources/images/transparent.gif" alt="u153_start"/>
              <img id="u153_end" class="img " src="/resources/images/transparent.gif" alt="u153_end"/>
              <img id="u153_line" class="img " src="/images/manage/u8_line.png" alt="u153_line"/>
            </div>
            </div></div></div>
            
</div>